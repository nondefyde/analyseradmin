...
$(function () {
    ...
    //Delete Hansard
    $(document.body).on('click', '.delete_hansard', function (e) {
        e.preventDefault();
        ...
        $("#hansard_name").text(' Vol: ' + vol + ', No: ' + no + ', House: ' + house + ', ' + sess);
        $("#confirm_hansard_delete").val($(this).val());
        box.addClass("open");
    });

    $(document.body).on('click', '#confirm_hansard_delete', function (e) {
        //e.preventDefault();
        var box = $("#confirm-remove-row");
        var hansard_id = $(this).val();
        $.ajax({
            type: 'GET',
            async: true,
            url: '/hansards/delete/' + hansard_id,
